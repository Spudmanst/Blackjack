...
def player_actions(player, cards, charlie_on):
    ...
    if player.hand1_stand == False:
        ...
        text_effect.slow_type(
            f"---Player {player.name}'s second hand ---"
        )
        text_effect.slow_type(
            f"Player {player.name}'s hand: {hand[0]} and {hand[1]}\nHand score: {score}"
        )
    
    # If a player has blackjack at start of round then they cannot split,
    # therefore if player reaches this check then they must have split first    
    if score == 21:
        ...
        while score < 21:
            action = text_effect.slow_input(
                f"What would you like to do, '(H)it'{", '(D)ouble' " if player.dbl_avail else ""}{", 'S(p)lit' " if player.split_avail else ""} or '(S)tand'? "
            ).lower()
            ...
            if action in ("hit", "h") or (player.dbl_avail and action in ("double", "d")):
                ...
                if charlie_on == True:
                    if win_check.charlie_check(hand, score) == "Charlie":
                        win = "Charlie"
                        text_effect.slow_type(
                            f"Player {player.name} has 5-Card Charlie!"
                        )
                        break
                ...
                text_effect.slow_type(
                    f"Player {player.name} stands with a score of {score}"
                )
                break
                ...
                print(
                    f"Unknown command, please type an option to continue.\n"
    ...
    # set variables back to Player now turn is over.
    if player.hand1_stand == False:
        player.hand1 = hand
...
def start_game(): 
    ...
    while True:
        try:
            ...
            if MIN_PLAYERS <= num_of_players <= MAX_PLAYERS:
                ...
                print(
                    f"Invalid input, must use a number between {MIN_PLAYERS} - {MAX_PLAYERS} to continue."
                )
        except ValueError:
            print(
                f"Invalid input, please enter a number between {MIN_PLAYERS} and {MAX_PLAYERS}."
            )
    
    while True:
        try:       
            named_players = text_effect.slow_input(
                "Are players going to give names, Yes or No?: "
            ).lower()
    ...
    for player_num in range(1, num_of_players + 1):
        while True: # While loop allows us to constantly ask for name if duplicate with another.
            if named_players in ("yes", "y"):
                name = text_effect.slow_input(
                    f"Enter the name for Player {player_num}: "
                ) 
    ...
    while round_number <= num_of_rounds:
        ...
        for player in players:
            new_round(player) # reset all players to ensure no issues going forward.
            ...
            if player.in_game:
                while True:
                    try:
                        ...
                        if bet > player.cash:
                            print(
                                f"Not enough funds, maximum available to Player {player.name} is {text_effect.format_cash(player.cash)}."
                            )
                        elif bet < 1:
                        ...
                        print(
                            f"Invalid input, please enter a whole number between $1.00 and {text_effect.format_cash(player.cash)}."
                        )
        ...
        if ins_active and dealer_upcard == "Ace":
                ...
                if player.active:
                        ...
                        try:
                            while True:
                                ...
                                if action in ("yes", "y"):
                                    ...
                                    if win_check.dealer_has_blackjack:
                                        player.win1 = "Push"
                                    break
                                else:
                                    text_effect.slow_type(
                                        "Unknown command, please type 'yes' or 'no' to proceed."
                                    )
                        except Exception as e:
                    ...
                    # If player doesn't have Blackjack then offer insurance if they can afford it.
                        ...
                        if insure_cost < player.cash:
                            ...
                            text_effect.slow_type(
                                f"Dealer's upcard is an ace, however you do not have enough funds to insure."
                            )
            ...
            if win_check.dealer_has_blackjack:
                text_effect.slow_type(
                    f"Dealer has {dealer_hand[0]} and {dealer_hand[1]}. Blackjack!"
                )
                for player in players:
                    if player.win1 != "Push":
        ...
        # if we don't have insurance, then we can speed up the game
            ...
            if win_check.dealer_has_blackjack == True:
                text_effect.slow_type(
                ...
                for player in players:
                    if player.active:
                        text_effect.slow_type(
                            f"Player {player.name}'s hand: {player.hand1[0]} and {player.hand1[1]}"
                        )
                        if player.win1 == "Blackjack":
                            text_effect.slow_type(
                                f"Player {player.name} has Blackjack!"
                            )
                            player.win1 = "Push"
                        else:
                            player.win1 = "Dealer_blackjack"
            ...
            # If all active players have Blackjack, then skip normal game process. We know dealer doesn't have Blackjack from "if" statement.
            elif all_players_blackjack and not win_check.dealer_has_blackjack:
                for player in players:
                    if player.active:
                        text_effect.slow_type(
                            f"Player {player.name}'s hand: {player.hand1[0]} and {player.hand1[1]}"
                ...
                if num_of_players != 1:
                    text_effect.slow_type(
                        "All participating players have Blackjack. What about the dealer?"
                    )
                else:
                    text_effect.slow_type(
                        f"Player {player.name} has Blackjack. What about the dealer?"
                    )
        ...
        if (ins_active and win_check.dealer_has_blackjack == False) or win_check.dealer_has_blackjack == False:
            ...
            for player in players:
                if player.active:
                    ...
                    text_effect.slow_type(
                        f"Dealer's hand: {dealer_hand[0]} and unknown"
                    )
                    text_effect.slow_type(
                        f"Player {player.name}'s hand: {player.hand1[0]} and {player.hand1[1]}"
                    )
            ...
            if all_players_out:
            ...
            elif not ins_active and all_players_blackjack and win_check.dealer_has_blackjack == False:
                continue
            else:
                text_effect.slow_type(
                    f"Dealer reveals hand: {dealer_hand[0]} and {dealer_hand[1]}"
                ...
                if ins_active and win_check.dealer_has_blackjack == True:
                    ...
                    if all_players_blackjack:
                        text_effect.slow_type(
                            "All active players have Blackjack, Dealer cannot match."
                        )
                    else:
        ...
        for player in players:
            if player.in_game:
                win_check.winner(dealer_score, player, hand = 1) # Prints results to screen
                win = win_check.player_payout(player, hand = 1)
                if win >= 0:
                    player.cash += win + player.bet + player.bet_dbl # must remember to give back their bet if they won
                if len(player.hand2) != 0:
                    win_check.winner(dealer_score, player, hand = 2)
                    win = win_check.player_payout(player, hand = 2)
                    if win >= 0:
        ...
        if no_one_can_bet:
            text_effect.divide_lines()
            text_effect.slow_type(
                "No players have any cash to bet with! Ending game."
            )
            text_effect.divide_lines()
            break
        else:        
    ...
    if round_number > num_of_rounds:
        ...
        for player in sorted_players:
            text_effect.slow_type(
                f"Player {player.name}: ${player.cash:.2f}"
            )
        text_effect.divide_lines()